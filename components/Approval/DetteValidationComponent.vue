          ...
          <div class="col-xl-4 col-sm-6" v-for="(dette, key) in dettes" :key="key">
            <div class="card">
                  ...
                  <div class="flex-shrink-0 align-self-center me-4">
                    <div class="avatar-md">
                      <span class="avatar-title badge-primary-subtle rounded-circle text-danger font-size-16">
                        <el-tooltip v-if="dette?.audit" :content="`L'utilisateur: ${dette?.audit.user.name}`"
                          placement="top">
                          <el-image class="bg-transparent avatar-image" fit="fill" :src="dette?.audit.user.photo"
                            :preview-src-list="[dette?.audit.user.photo]" lazy>
                            <template #error>
                        ...
                        <el-tooltip v-else :content="`L'utilisateur: ${dette?.audit.user.name}`" placement="top">
              ...
              <div class="d-flex border-top justify-content-center px-1 py-1">
                <el-button @click="runShowModal(dette?.id!)" class="w-100" type="primary" plain>consulter et
                  valider</el-button>
              </div>
            </div>
          </div>
          <LazyDetteShowModal :id="show.id" v-model="show.enable" v-if="show.enable" :from-validation-page="true" />
        </div>
      </el-scrollbar>
      <el-empty v-else :image-size="80">
        <template #description>
          <el-text class="headline">aucune dette à remboursser</el-text>
        </template>
      </el-empty>
    </div>
  </div>
