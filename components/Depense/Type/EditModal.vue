...
const { update, getOne } = useTypeDepenseStore();
const { type, loading } = storeToRefs(useTypeDepenseStore());
...
<template>
  <Form class="form-horizontal" @submit="onSubmit" :initial-values="type" v-slot="{ isSubmitting, errors }">
    <el-dialog v-model="dialog" title="Modifier le type de dépense" width="35%" destroy-on-close center>
