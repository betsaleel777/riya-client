...
const { create } = useProprietaireStore();
...
<template>
  <Form class="form-horizontal" @submit="onSubmit" v-slot="{ isSubmitting, errors }">
    <el-dialog v-model="dialog" title="Créer propriétaire" width="35%" destroy-on-close center>
      <ProprietaireDialogForm v-loading="isSubmitting" :errors="errors" />
      <template #footer>
