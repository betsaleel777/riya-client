...
import { statusValidable, statusAvance, rolesNames } from "~/utils/constante";
            ...
            <div class="card-body">
              <StructurePageHeader :breadcrumbs="links" title="Locations">
                ...
                <el-row class="mt-1 mb-2" justify="end">
                  <el-col :span="24">
                    <el-input class="w-50" v-model="search" placeholder="code, client, statut, statut avance" />
                  </el-col>
                </el-row>
                <el-table v-loading="loading.index" :data="filterTableData" class="w-100" empty-text="aucune Location">
                  <el-table-column prop="code" label="Code" width="100" />
                  <el-table-column show-overflow-tooltip prop="personne" label="Client" />
                  <el-table-column prop="frais" label="Frais agence" align="center" width="150" sortable>
                    <template #default="scope">
                      {{ useCurrency(scope.row.frais * scope.row.loyer) }}
                    </template>
                  </el-table-column>
                  <el-table-column prop="caution" label="Caution" align="center" width="150" sortable>
                    <template #default="scope">
                  ...
                  <el-table-column prop="avanceStatus" label="Statut avance" width="150">
                    <template #default="scope">
                      <el-tag class="text-truncated" :type="classStatus(scope.row.avanceStatus) as Variant">{{
              ...
              <VisiteEditModal :id="modal.edit.id" v-if="modal.edit.dialog" v-model="modal.edit.dialog" />
              <VisiteShowModal :id="modal.show.id" v-if="modal.show.dialog" v-model="modal.show.dialog" />
            </div>
          </div>
        </div>
      </div>
